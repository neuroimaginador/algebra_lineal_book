...
- [¿Qué es un subespacio vectorial?](#subesp)
- [¿Qué es un sistema de vectores linealmente independiente?](#indep)
- [¿Qué es un sistema generador y el subespacio generado por un conjunto de vectores?](#gen)
...
- [¿Es la unión de subespacios un subespacio?](#union)
- [¿Cómo hallar la suma de dos subespacios vectoriales?](#suma)
- [¿Qué dice el teorema de la dimensión?](#th-dim)
- [¿Cómo calcular el subespacio suplementario de uno dado?](#supl)
...
## Qué es un subespacio vectorial {#subesp}
\sectionmark{Subespacio Vectorial}

```{r}
definition("Un subconjunto de $U\\subset V$ es un subespacio vectorial de $V$ si la restricción de las operaciones suma y producto por un escalar a ese conjunto son operaciones cerradas.", "Subespacio vectorial", "subespacio vectorial")
```


Eso equivale a decir que la suma de dos vectores cualesquiera de $U$ debe estar en $U$, y si multiplicamos por un escalar un vector de $U$, el producto vuelve a estar en $U$.
...
## Qué es un sistema de vectores linealmente independiente {#indep}
\sectionmark{Independencia lineal}

```{r}
definition("Supongamos un conjunto de vectores $\\{v_1,\\ldots,v_n\\}$. Este conjunto es linealmente independiente si ninguno de los vectores se puede poner como combinación lineal de los demás vectores.", "Sistema linealmente independiente", "linealmente independiente")
```
...
```{r}
theorem("Si el número de ecuaciones (filas) es menor estrictamente que el número de incógnitas (columnas), entonces el sistema es compatible indeterminado. En otro caso (mismo número de ecuaciones que de incógnitas), será compatible determinado.")
```
...
## Qué es un sistema generador y el subespacio generado por un conjunto de vectores {#gen}
\sectionmark{Sistema generador} 

En esta sección, estudiamos una noción complementaria a la de [sistema linealmente independiente](#indep).

```{r}
definition("Consideramos un sistema de vectores $\\mathcal{S} = \\{v_1,\\ldots,v_n\\}$. El __subespacio generado__ por $\\mathcal{S}$ es el conjunto de vectores que se calcula como combinaciones lineales de los de $\\mathcal{S}$.", "Subespacio generado")
```

Es decir, si vamos dando valores a unos escalares $\alpha_1,\ldots,\alpha_n$, entonces el vector $\alpha_1v_1+\ldots+\alpha_nv_n$ es combinación lineal de los de $\mathcal{S}$, luego pertenece al subespacio generado por éste.

Como conclusión, el subespacio generado por $\mathcal{S}$ es el menor subespacio de $V$ que contiene a los elementos de $\mathcal{S}$. 
...
```{r}
definition("Para cualquier espacio vectorial, o subespacio, una base es un sistema de vectores que [lo genera](#gen) (mediante combinaciones lineales, como hemos visto antes) y que es [linealmente independiente](#indep).", "Base", "base")
```
...
```{r}
theorem("Todo sistema generador de un subespacio contiene un sistema linealmente independiente, que también genera el mismo subespacio.")
```
...
## Cómo encontrar la base (y la dimensión) para un subespacio vectorial {#base}
\sectionmark{Bases de subespacios}
...
## Cómo convertir entre ecuaciones cartesianas y paramétricas de un subespacio vectorial {#param2cartesian}
\sectionmark{Ecuaciones cartesianas y paramétricas}
...
for (p in seq_along(L$eq_idx[-1])) {
  ...
  for (i in seq(nrow(par))) {
    
    this_par <- par[i, ] %>% matrix(nrow = 1)
    this_eq <- eqs[i, ] %>% matrix(nrow = 1)
    
    str_p <- c(str_p,
               write_system(this_par, 
                            fractions = TRUE,
                            latex = TRUE,
                            format = 'c', 
                            vars = params) %>% 
  ...
  str_p <- str_p %>% matrix(ncol = 1)
  str_a <- str_a %>% matrix(ncol = 1)
  ...
  glue("- Despejamos ${params[p]}$ de la ecuación {L$eq_idx[p]}, y sustituimos en las demás, lo cual nos da:\n",
       "$${glue_matrices(str_a, equals, str_p, latex = TRUE, ldeco = '', rdeco = '', sep = '', format = 'rcl')}$$\n"
       ) %>% cat()
  cat("\n")
...
## Cómo hallar la intersección de dos subespacios vectoriales {#interseccion}
\sectionmark{Intersección de subespacios}
...
```{r}
theorem("La intersección de dos subespacios es un nuevo subespacio, aunque puede suceder que la intersección sea únicamente el vector 0.")
```
...
## ¿Es la unión de subespacios un subespacio? {#union}
\sectionmark{Unión de subespacios}
...
## Cómo hallar la suma de dos subespacios vectoriales {#suma}
\sectionmark{Suma de subespacios}

Aparte de la [intersección](#interseccion) y de la [unión](#union) de subespacios vectoriales, podemos definir una nueva operación:

```{r}
definition("Dados dos subespacios $U$ y $W$ de $V$, se define su suma $U+W$ como el conjunto de vectores que son combinación lineal de un vector de $U$ y otro de $W$.", "Suma de subespacios", "suma")
```

Si, además, $U\cap W = \{0\}$ y $U+W = V$, entonces se llama __suma directa__ y se denota por $U\oplus W$. Se dice entonces que $U$ es el __subespacio suplementario__ de $W$ y viceversa.

El resultado teórico esencial es el siguiente:

```{r}
theorem("Si $\\mathcal{B}_U$ y $\\mathcal{B}_W$ son bases de $U$ y $W$ respectivamente, entonces $\\mathcal{G} = \\mathcal{B}_U \\cup \\mathcal{B}_W$ es un sistema generador de $U+W$.")
```
...
```{r}
theorem("$\\mathrm{dim}(U+W) = \\mathrm{dim}(U) + \\mathrm{dim}(W) - \\mathrm{dim}(U\\cap W)$.", "Fórmula de Grassmann")
```
...
## Cómo calcular el subespacio suplementario de uno dado {#supl}
\sectionmark{Subespacio suplementario}
...
## Cómo calcular las coordenadas de un vector con respecto a una base dada {#coord}
\sectionmark{Coordenadas}

Un concepto importante es el de __coordenadas__ de un vector, asociadas a una base $\mathcal{B}$ del espacio vectorial $V$.

```{r}
definition("Supongamos una base $\\mathcal{B} = \\{v_1,\\ldots,v_n\\}$ de $V$. Las __coordenadas__ de un vector $v$ son unos escalares $\\alpha_1,\\ldots,\\alpha_n$ tales que podemos expresar $v$ como una combinación lineal de los vectores de la base con esos coeficientes:\n
$$v = \\alpha_1 v_1 + \\cdots + \\alpha_n v_n$$", "Coordenadas")
```
...
## Cómo calcular la matriz de cambio de base entre dos bases de un mismo espacio vectorial {#cambiobase}
\sectionmark{Matriz de cambio de base}
...
```{r}
theorem("La transformación de las coordenadas para un mismo vector $v$ entre dos bases $\\mathcal{B}_1$ y $\\mathcal{B}_2$ viene dada por la siguiente expresión:\n
